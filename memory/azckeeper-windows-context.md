...
> - **PROGRESO panel 2026-06-18 (en PROD keep.azclegal.com):** ✅ #1 backfill correo (15 filas). ✅ #2 columna `keeper_users.employment_status` + sync desde `employee.role` (21 retired) + UI badge/selector. ✅ #3 `keeper_devices.decommission_reason`+`decommissioned_at` + obsolescencia 30d + baja manual (devuelto/cambiado). ✅ #6/#7 users.php (perf 2 fases + filtros clicables). Código en DevLinux (commits 49cb1e5, 45b62f4, 2a050e1); desplegado MANUAL (scp) a keep.azclegal.com/public/. Rollbacks en server `~/backups_keeper/` y local `/home/kelsie/azc_backups/`. PENDIENTE: #8 mapa admins, #4 festivos (no existía, construir de cero), #5 logs por usuario (requiere reporte cliente→servidor de updates).
>
> **PENDIENTE — RETOMAR LUNES 2026-06-22: optimización para 1000 usuarios concurrentes.** Review hecho 2026-06-18 (3 análisis de código + sondeo BD en vivo). Hallazgos para NO re-investigar:
> - Carga cliente a 1000: ~70 req/s, ~900 KB/s; **86% = batches window-episode**. Intervalos reales: handshake 300s(min120)/activity 30s/episodes 30s, TODOS con jitter (bien). Único sin jitter: cola offline (retry fijo) → thundering herd al recuperar red. Refs CoreService.cs.
> - BD viva (MySQL 8.0.40): max_connections=10000 ok; wait_timeout=60, thread_cache_size=8, table_open_cache=256 (bajos); **Aborted_connects=274794 (investigar)**. keeper_window_episode 1.23M filas, 6 índices.
> - **Riesgo #1 = HOSTING**: prod en cPanel COMPARTIDO (server1872) topa límite de procesos/forks → no sostiene 1000 concurrentes (cuello en capa PHP, no BD). Mover backend a box dedicado (azc.com.co Hestia 8GB o cloud.azclegal.com 94GB) + PHP-FPM 100-200 workers + OPcache. DECISIÓN DE INFRA del usuario.
> - **Riesgo #2 = write-amp keeper_window_episode**: ~4M filas/mes a 1000 users, 6 índices/INSERT, sin retención. Plan: rollup nocturno `keeper_app_daily` + DELETE/partición por day_date >180d; quitar índices baja-selectividad (ix_keeper_we_start, ix_keeper_we_process) y agregar (day_date,process_name)+(day_date,start_at).
> - **Orden acordado:** (1) QUICK WINS por mí (seguro, con backup): Cache-Control en ClientVersion.php; índices nuevos window_episode + keeper_audit_log(event_type,created_at) + keeper_user_assignments(keeper_user_id,sede_id,firm_id). (2) Proyecto BD: retención/rollup. (3) Infra: box dedicado + tuning. (4) Build cliente: jitter en cola offline. Panel pendiente (index.php/sedes-dashboard.php): agregaciones pesadas sobre window_episode (top apps, MIN start_at, ocio LIKE) → usar índices nuevos + keeper_focus_daily.distraction_seconds en vez de LIKE.
