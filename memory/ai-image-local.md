...
- **Acceso por Tailscale (tablet)**: **`https://fumilinux.tail7024a4.ts.net/`** (HTTPS:443, hostname MagicDNS, NO IP). `tailscale serve --bg 8769` → cert **real Let's Encrypt** (se habilitó HTTPS en la consola admin de Tailscale 06-17; el error previo "account does not support TLS certs" era solo ese toggle apagado, NO limitación de plan). Contexto seguro → **PWA instalable sin instalar CA** (verificado: isSecureContext, SW activo). `operator=kelsie`. Quedó un fallback self-signed dormido en dashboard.py (`IAGEN_SELF_HTTPS=1`, CA local en ~/.config/iagen/certs) por si hiciera falta. **Nextcloud snap retirado** (ocupaba puerto 80, 12K de datos, snapshot guardado). Ver [[reference_tailscale-dashboard-access]].

## Dashboard: rendimiento + biblioteca + reroll + visor (2026-06-17)

Continuación. Detalle en `project_dashboard-control-panel` ⑰. Resumen:

- **Rendimiento (la galería se sentía lenta en tablet):** el server era **HTTP/1.0** → sin keep-alive, TLS nuevo por miniatura. `protocol_version="HTTP/1.1"` → medido **92 ms/thumb → 4 ms/thumb (~23×)**. La tablet conecta DIRECTO en la LAN (no relay), así que era protocolo, no red. **Tier `/view`** (WebP 1280px ~68 KB) para el visor en vez del PNG 1.8 MB (**26×**); el modal muestra la miniatura al instante + precarga vecinos. `content-visibility:auto`+`min-width:0` en cards (sin el min-width, columnas fijas desbordaban en escritorio). Acceso por IP Tailscale sin DNS: listener extra en la IP 100.x (`http://100.116.50.47:8769/`).
- **📚 Biblioteca** = tab de nivel superior (junto a 🖼️ Imágenes generadas, default): personajes (lista, copiar agrupados) + prompts guardados, server-side `library.json`, compartido tablet/PC. Prompt "✨ Embellecido / calidad" con tags de sombras/luz/detalle.
